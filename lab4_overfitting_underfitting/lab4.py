...
def compute_learning_curves(X_train, y_train, X_val, y_val, lamb):
    ...
    for i in range(1, m):
        ...
        # ВАШ КОД ЗДЕСЬ
        opt_theta = train_linear_regression(X_train[:i, :], y_train[:i], init_theta, lamb)[1]
        print(f'шаг {i}: h(x): {opt_theta}')
...
poly_lamb = 0.5  # параметр регуляризации для полиномиальной регрессии, можно варьировать
